...
def convert(sentence, label, max_length, zero_vector, zero_label):
    if len(sentence) >= max_length:
        return (sentence[:max_length], label[:max_length], [True]*max_length)
    else:
        l = len(sentence)
        return (sentence + [zero_vector]*(max_length-l), label + [zero_label]*(max_length-l),
                [True]*l + [False]*(max_length-l))
...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ret.append(convert(sentence, labels, max_length, zero_vector, zero_label))
    return ret
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        return tf.reshape(embeddings, (-1, self.max_length, self.config.n_features*self.config.embed_size))
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("RNN") as scope:
            for time_step in range(self.max_length):
                ...
                h, _ = cell(x[:, time_step, :], h, scope)
                o = tf.nn.dropout(h, keep_prob=1 - Config.dropout)
    ...
    def add_loss_op(self, preds):
        ...
        """
        return tf.reduce_mean(tf.boolean_mask(tf.nn.sparse_softmax_cross_entropy_with_logits
            (labels=self.labels_placeholder, logits=preds), self.mask_placeholder))
